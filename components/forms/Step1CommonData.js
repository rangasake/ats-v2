...
export default function Step1CommonData({ data, onSave, loading }) {
  ...
  const [otherMandal, setOtherMandal] = useState(
    data?.mandal_name && !MANDAL.mandals.includes(data.mandal_name)
      ? data.mandal_name
      : "",
  );
  ...
  const [dbFields, setDbFields] = useState(() => {
    ...
    return new Set(
      DB_VEHICLE_FIELDS.filter((k) => data[k] !== "" && data[k] != null),
    );
  });
  const [vehicleHistory, setVehicleHistory] = useState([]);

  function isLocked(field) {
    return dbFields.has(field);
  }
  ...
  function set(field, value) {
    setForm((prev) => {
      ...
      if (field === "mandal_name") {
        next.rto_office = MANDAL?.mandalRtoMap[value] || "";
      }
      return next;
    });
    setErrors((e) => ({ ...e, [field]: "" }));
  ...
  async function searchVehicle() {
    ...
    try {
      ...
      const mandalsRes = await fetch("/api/config/mandals");
      const mandals = await mandalsRes.json();
      setMANDAL(mandals || {});
      console.log(mandals);
      ...
      if (json.found) {
        setForm((prev) => ({
          ...prev,
          ...json.vehicle,
          registration_date: normalizeDateForInput(
            json.vehicle?.registration_date,
          ),
        }));
        // Lock all non-empty DB fields (meter_reading always stays editable)
        setDbFields(
          new Set(
            DB_VEHICLE_FIELDS.filter(
              (k) => json.vehicle[k] !== "" && json.vehicle[k] != null,
            ),
          ),
        );
  ...
  function validate() {
    ...
    if (form.mandal_name === "OTHER" && !otherMandal.trim())
      err.mandal_name = "Please enter the mandal name";
    ...
    if (validate()) {
      const finalForm =
        form.mandal_name === "OTHER"
          ? {
              ...form,
              mandal_name: otherMandal.trim(),
              rto_office: otherMandal.trim(),
            }
          : form;
      onSave(finalForm);
    }
      ...
      <div className="card mb-4">
        ...
        {noVehicleFound && (
          <p className="text-red-500 text-xs my-4">
            Vehicle number not avaliable in database enter vehicle deatils as
            per original documents{" "}
          </p>
        )}
        ...
        <>
          ...
          {vehicleHistory.length > 0 && (
            <div className="card mb-4">
              <h2 className="section-title">
                📜 Past Inspections ({vehicleHistory.length})
              </h2>
            ...
            <div className="mb-4">
              ...
              {isLocked("mandal_name") ? (
                ...
                <>
                  <select
                    value={
                      MANDAL.mandals.includes(form.mandal_name)
                        ? form.mandal_name
                        : form.mandal_name
                          ? "OTHER"
                          : ""
                  ...
                  {(form.mandal_name === "OTHER" ||
                    ...
                    <div className="mt-2">
                      ...
                      <input
                        ...
                        value={otherMandal}
                        onChange={(e) => {
                          setOtherMandal(e.target.value);
                          setErrors((err) => ({ ...err, mandal_name: "" }));
                        }}
                        className="form-input"
                      />
                    </div>
          ...
          <div className="card mb-4">
            ...
            {isLocked("lane_type") ? (
              <div className="mb-4">
                <label className="form-label">
                  Vehicle Lane Type <span className="text-red-500">*</span>
                </label>
