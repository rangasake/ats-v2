...
import { MANDALS, MANDAL_RTO_MAP, VEHICLE_LANES, LANE_TYPES } from '../../lib/constants';
      ...
      <div className="card mb-4">
        ...
        <div className="flex gap-2">
          ...
          {!searchDone ? (
            ...
            <button
              type="button"
              onClick={() => { setSearchDone(false); setForm((p) => ({ ...p, vehicle_number: '' })); }}
      ...
      {searchDone && (
        <>
          ...
          <div className="card mb-4">
            ...
            <Field label="Engine Number" value={form.engine_number} onChange={(v) => set('engine_number', v)} error={errors.engine_number} required />
            <Field label="Chassis Number" value={form.chassis_number} onChange={(v) => set('chassis_number', v)} error={errors.chassis_number} required />
            <div className="mb-4">
              <label className="form-label">Registration Date <span className="text-red-500">*</span></label>
              <input type="date" value={form.registration_date} onChange={(e) => set('registration_date', e.target.value)} className="form-input" />
          ...
          <div className="card mb-4">
            ...
            <Field label="Owner Name" value={form.owner_name} onChange={(v) => set('owner_name', v)} error={errors.owner_name} required />
            <div className="mb-4">
              <label className="form-label">Owner Phone <span className="text-red-500">*</span></label>
              <input type="tel" value={form.owner_phone} onChange={(e) => set('owner_phone', e.target.value)} className="form-input" maxLength={10} />
            ...
            <div className="mb-4">
              ...
              <select value={form.mandal_name} onChange={(e) => set('mandal_name', e.target.value)} className="form-input">
                <option value="">Select Mandal</option>
                {MANDALS.map((m) => <option key={m} value={m}>{m}</option>)}
              </select>
              {errors.mandal_name && <p className="text-red-500 text-xs mt-1">{errors.mandal_name}</p>}
            </div>
            <div className="mb-4">
              <label className="form-label">RTO Office</label>
              <input type="text" value={form.rto_office} readOnly className="form-input bg-gray-50 text-gray-500" placeholder="Auto-filled from Mandal" />
            </div>
          </div>
          ...
          <div className="card mb-4">
            ...
            <div className="mb-4">
              ...
              <select value={form.vehicle_lane} onChange={(e) => set('vehicle_lane', e.target.value)} className="form-input">
                <option value="">Select Lane</option>
                {VEHICLE_LANES.map((l) => <option key={l} value={l}>{l}</option>)}
              </select>
              {errors.vehicle_lane && <p className="text-red-500 text-xs mt-1">{errors.vehicle_lane}</p>}
            </div>
            <SearchableDropdown
              label="Lane Type"
              options={LANE_TYPES}
              ...
              required
            />
            {errors.lane_type && <p className="text-red-500 text-xs mt-1">{errors.lane_type}</p>}
          </div>
        </>
      )}

      {searchDone && (
        <button type="button" onClick={handleSave} disabled={loading} className="btn-primary mt-2">
