...
async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    try {
      const agent = await findRow(req.user.orgId, SHEETS.AGENTS, 'phone', phone.trim());
  ...
  if (req.method === 'POST') {
    ...
    try {
      const existing = await findRow(req.user.orgId, SHEETS.AGENTS, 'phone', phone.trim());
      if (existing) return res.status(409).json({ error: 'Agent already exists' });
      await appendRow(req.user.orgId, SHEETS.AGENTS, { phone: phone.trim(), name: name.trim() });
