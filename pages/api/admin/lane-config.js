...
async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      const rows = await getRows(req.user.orgId, SHEETS.LANE_CONFIG);
  ...
  if (req.method === 'POST') {
    ...
    try {
      const existing = await findRow(req.user.orgId, SHEETS.LANE_CONFIG, 'lane_type', lane_type);
      const payload = {
        lane_type,
      ...
      if (existing) {
        await updateRow(req.user.orgId, SHEETS.LANE_CONFIG, 'lane_type', lane_type, payload);
      } else {
        await appendRow(req.user.orgId, SHEETS.LANE_CONFIG, payload);
      }
