...
export async function appendRows(spreadsheetId, sheetName, rowDataList) {
  if (!rowDataList.length) return;
  const headers = await getHeaders(spreadsheetId, sheetName);
  const values = rowDataList.map((d) => headers.map((h) => d[h] ?? ''));
  const sheets = getSheetsClient();
  await sheets.spreadsheets.values.append({
    spreadsheetId: spreadsheetId,
    range: sheetName,
...
// ─── Delete every row matching a key value ────────────────────────────────────
export async function deleteRowsByValue(spreadsheetId, sheetName, keyCol, keyVal) {
  const sheets = getSheetsClient();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: spreadsheetId,
    range: sheetName,
  ...
  if (keyIdx === -1) return { deleted: 0 };

  const needle = String(keyVal).trim().toUpperCase();
  const keep = [headers];
  let deleted = 0;
  values.slice(1).forEach((row) => {
    if (String(row[keyIdx] || '').trim().toUpperCase() === needle) deleted++;
    else keep.push(row);
  });
  ...
  if (deleted > 0) {
    ...
    // Clear any leftover old rows below the rewritten data
    await sheets.spreadsheets.values.clear({
      spreadsheetId: spreadsheetId,
      range: `${sheetName}!A${keep.length + 1}:${lastCol}10000`,
    });
    invalidateCache(spreadsheetId, sheetName);
  }
  return { deleted };
...
// ─── Update ALL rows matching a key column ────────────────────────────────────
export async function updateRowsByValue(spreadsheetId, sheetName, keyCol, keyVal, updates) {
  const sheets = getSheetsClient();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: spreadsheetId,
    range: sheetName,
  ...
  rows.slice(1).forEach((row) => {
    if (String(row[keyIdx] || '').trim().toUpperCase() === needle) {
      updatedRows.push(headers.map((h, i) => updates[h] !== undefined ? updates[h] : (row[i] ?? '')));
    }
  });

  if (updatedRows.length === 0) return 0;

  // Rebuild the full sheet data, replacing matching rows
  const allData = [headers];
  let updateIdx = 0;
  rows.slice(1).forEach((row) => {
    if (String(row[keyIdx] || '').trim().toUpperCase() === needle) {
      allData.push(updatedRows[updateIdx++]);
    } else {
      allData.push(row);
    }
  });
